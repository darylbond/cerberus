    ...
    for (int i=0; i<n_species; ++i) {
        ...
        for (int s=0; s<num_states; ++s) {
            ...
            if (found.first) {
                ...
                info.m = states[s]->mass[info.alpha_idx];
                info.q = states[s]->charge[info.alpha_idx];
                break;
            }
        }
...
void Collisions::calc_time_derivative(MFP* mfp, Vector<UpdateData>& update, const Real time, const Real dt)
{
    ...
    for (size_t i=0; i<n_states;++i) {
        ...
        accounting[i].resize(hstate.n_species);
        alpha[i].resize(hstate.n_species);
    }
                ...
                AMREX_PRAGMA_SIMD
                    ...
                    for (int si=0; si<n_states; ++si) {
                        ...
                        if (Un[+HydroDef::ConsIdx::Density] < states[si]->effective_zero) {
                            ...
                            states[si]->cons2prim(U[si], Q[si]);
                            states[si]->get_alpha_fractions_from_prim(Q[si], alpha[si]);
                        }
                    }
                    ...
                    for (int n=0; n<n_species; ++n) {
                        ...
                        if (Un[+HydroDef::ConsIdx::Density] > states[info.state_idx]->effective_zero) {
                            ...
                            info.m = states[n]->mass[info.alpha_idx];
                            info.q = states[n]->charge[info.alpha_idx];
                            info.T = states[n]->get_temperature_from_prim(Qn);
