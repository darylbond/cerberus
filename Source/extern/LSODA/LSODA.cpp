...
/* Purpose : Find largest component of Real Vector dx */
size_t LSODA::idamax1( const Vector<Real>& dx, const size_t n_, const size_t offset=0)
{
...
void LSODA::daxpy1(const Real da, const Vector<Real> &dx,
                   Vector<Real> &dy, const size_t n_, const size_t offsetX = 0,
...
void LSODA::dgesl(const Vector<Vector<Real>> &a, const size_t n_,
                  Vector<int> &ipvt_, Vector<Real> &b, const size_t job)
{
    size_t k, j;
    Real t;
...
// See BLAS documentation. All Real* has been changed to Vector .
void LSODA::dgefa(Vector<Vector<Real>> &a, const size_t n_, Vector<int> &ipvt_,
                  size_t *const info)
{
    size_t j = 0, k = 0, i = 0;
    Real t = 0.0;
...
void LSODA::lsoda(LSODA_ODE_SYSTEM_TYPE f, const size_t neq, Vector<Real> &y,
                  Real *t, Real tout, int itask, int *istate, int iopt,
                  int jt, array<int, 7> &iworks, array<Real, 4> &rworks)
{
    assert(tout > *t);
    ...
    */
    if (*istate == 1)
    {
        ...
        */
        ...
        yh_.resize(lenyh + 1, Vector<Real>(nyh + 1, 0.0));
        wm_.resize(nyh + 1, Vector<Real>(nyh + 1, 0.0));
        ewt.resize(1 + nyh, 0);
        savf.resize(1 + nyh, 0);
...
void LSODA::stoda(const size_t neq, Vector<Real> &y, LSODA_ODE_SYSTEM_TYPE f)
{
    assert(neq + 1 == y.size());
    ...
    while (1)
    {
        while (1)
        {
            ...
            if (nst >= nslp + msbp)
                ipup = miter;
            tn_ += h_;
            for (size_t j_ = nq; j_ >= 1; j_--)
                for (size_t i1_ = j_; i1_ <= nq; i1_++)
                    for (i = 1; i <= n; i++)
                        yh_[i1_][i] += yh_[i1_ + 1][i];
...
void LSODA::cfode(int meth)
{
    ...
    if (meth == 1)
    {
        ...
        for (nq_ = 2; nq_ <= 12; nq_++)
        {
            ...
            */
            ...
            {
                tsign = -tsign;
                pint += tsign * pc[i_] / (Real)i_;
                xpin += tsign * pc[i_] / (Real)(i_ + 1);
            }
            /*
            ...
            */
            ...
            elco[nq_][2] = 1.;
            for (i_ = 2; i_ <= nq_; i_++)
                elco[nq_][i_ + 1] = rq1fac * pc[i_] / (Real)i_;
            agamq = rqfac * xpin;
            ragq = 1. / agamq;
            tesco[nq_][2] = ragq;
            if (nq_ < 12)
    ...
    */
    for (nq_ = 1; nq_ <= 5; nq_++)
    {
        ...
        */
        pc[nqp1] = 0.;
        for (i_ = nq_ + 1; i_ >= 2; i_--)
            pc[i_] = pc[i_ - 1] + fnq * pc[i_];
        pc[1] *= fnq;
        /*
           Store coefficients in elco and tesco.
        */
        for (i_ = 1; i_ <= nqp1; i_++)
            elco[nq_][i_] = pc[i_] / pc[2];
        elco[nq_][2] = 1.;
        tesco[nq_][1] = rq1fac;
        tesco[nq_][2] = ((Real)nqp1) / elco[nq_][1];
        tesco[nq_][3] = ((Real)(nq_ + 2)) / elco[nq_][1];
        rq1fac /= fnq;
    }
    return;
...
void LSODA::prja(const size_t neq, Vector<Real> &y, LSODA_ODE_SYSTEM_TYPE f)
{
...
*/
Real LSODA::vmnorm(const size_t n_, const Vector<Real> &v,
                     const Vector<Real> &w)
{
    Real vm = 0.;
    for (size_t i = 1; i <= n_; i++)
...
Real LSODA::fnorm(int n_, const Vector<Vector<Real>> &a,
                    const Vector<Real> &w)
...
void LSODA::correction(const size_t neq, Vector<Real> &y,
                       LSODA_ODE_SYSTEM_TYPE f, size_t *corflag, Real pnorm,
                       Real *del, Real *delp, Real *told, size_t *ncf,
                       Real *rh, size_t *m)
{
...
void LSODA::methodswitch(Real dsm, Real pnorm, Real *pdh, Real *rh)
{
    ...
    Real rh1, rh2, rh1it, exm2, dm2, exm1, dm1, alpha, exsm;
...
*/
void LSODA::orderswitch(Real *rhup, Real dsm, Real *pdh, Real *rh,
                        size_t *orderflag)
{
    size_t newq = 0;
...
{
    array<int, 7> iworks = {{0, 0, 0, max_step, 10, 100, 100}};
    ...
    // remove first element of yout
    yout.erase(yout.begin());
}
