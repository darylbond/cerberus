#include "MFP_udf_collisions.H"
#include "MFP.H"
#include "MFP_state.H"
#include "sol.hpp"

std::string CollisionsUDF::tag = "udf_collisions";
bool CollisionsUDF::registered = GetActionFactory().Register(CollisionsUDF::tag, ActionBuilder<CollisionsUDF>);

CollisionsUDF::CollisionsUDF(){}
CollisionsUDF::~CollisionsUDF(){}

CollisionsUDF::CollisionsUDF(const int idx, const sol::table &def) : Collisions(idx, def)
{
    ...
    if (cross_sections.empty()) {
        ...
        for (const SpeciesInfo& info : species_info) {
            if (states[info.state_idx]->charge[info.alpha_idx] != 0.0) {
...
void CollisionsUDF::calc_update()
{
